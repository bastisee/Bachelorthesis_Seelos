...
# --- Load CSV ---
file_path = "impact_data/512_40_shoot/impact_007.csv"
...
# --- True coords for error calculation ---
true_x = 65
true_y = 51
...
# --- Microphone positions (in cm) STAGE 1,2,3 ---
...
#cross correlation
def get_delay_range(sig_a, sig_ref, min_lag=-150, max_lag=150):
    corr = correlate(sig_a, sig_ref, mode='full', method='fft')
    lags = np.arange(-len(sig_a) + 1, len(sig_ref))
    # Find indices within lag range
    valid = (lags >= min_lag) & (lags <= max_lag)
    # Only consider correlation values within lag range
...
def estimate_aoa13_from_delay(tdoa_31, mic1, mic3):
    ...
    # Adjust angle based on delay
    aoa23_meas = mic_line_angle + np.arcsin(angle_offset)
    return aoa23_meas
...
#window detection
def detect_window(signal, threshold=0.2):
    peak_idx = np.argmax(np.abs(signal))
    start = max(0, peak_idx - 100)  # Adjust based on expected signal duration
    end = min(len(signal), peak_idx + 100)
    return start, end


    # Extract the window pattern from the reference signal (Mic1)
    reference_window = reference_signal[window_start:window_end]

    # Initialize variables to track the best alignment
    best_lag = None
    max_correlation = -np.inf

    # Slide the reference window across the target signal
    for lag in range(len(target_signal) - len(reference_window) + 1):
        # Extract the corresponding segment from the target signal
        target_segment = target_signal[lag:lag + len(reference_window)]

        # Compute the correlation between the reference window and the target segment
        correlation = np.correlate(reference_window, target_segment, mode='valid')[0]
...
window_start, window_end = detect_window(filtered['Mic1'])
...
#TDOA Calculation
delay_21 = get_delay_range(filtered['Mic2'], windowed['Mic1'], min_lag=-400, max_lag=400)  # Mic2 - Mic1
delay_31 = get_delay_range(filtered['Mic3'], windowed['Mic1'], min_lag=-400, max_lag=400)  # Mic3 - Mic1
...
plt.figure(figsize=(10, 4))
plt.plot(filtered['Mic1'], label='Filtered Mic1 (Reference)', linewidth=1)
...
plt.legend()
plt.title("Cross-Correlation: Mic2/Mic3 vs Mic1")
plt.xlabel("Lag")
plt.ylabel("Cross-Correlation")
plt.grid(True)
plt.tight_layout()
